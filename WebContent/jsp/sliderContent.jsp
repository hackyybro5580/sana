...
<table style="width: 100%;text-align: center;" id="productsTable">
	<tr>
		<th colspan="2"><button style="padding:5px 18px;" onclick="addSliderRow();"><i class="fa fa-plus"></i></button></th>
		<th>Path</th>
		<th>Title 1</th>
		<th>Title 2</th>
	</tr>
		...
		<%for(int i=0;i<productsArray.length();i++){JSONObject obj = productsArray.getJSONObject(i);%>
			<tr>
				...
				<td>
					...
					<span class="edit"><input type="text" selectorVal="path" value='<%=obj.get("path")%>'></span>
				</td>
				<td>
					<span class="default"><%=obj.get("title1")%></span>
					<span class="edit"><input type="text" selectorVal="title1" value='<%=obj.get("title1")%>'></span>
				</td>
				<td>
